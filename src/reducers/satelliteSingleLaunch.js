...
function calculate(state, body) {
    ...
    const transferPeriod = (orbit.period * (satelliteCount - 1)) / satelliteCount;
    const transferOrbit = Orbit.fromApAndPeriod(body, apoapsis, transferPeriod);

    return state.set('transferOrbit', transferOrbit)
                .set('errors', {});
}
...
export default function(state = initialState, action) {
    switch (action.type) {
        case 'SATELLITE.SINGLE_LAUNCH.FORM_UPDATE':
            return formUpdate(state, action.field, action.value);
        case 'SATELLITE.SINGLE_LAUNCH.CALCULATE':
